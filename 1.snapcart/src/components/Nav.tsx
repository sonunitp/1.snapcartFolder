...
setSearchBarOpen(false)
    ...
    const sideBar = menuOpen ? createPortal(
        <AnimatePresence>
            <motion.div
              ...
              bg-linear-to-b from-red-800/90 via-red-700/80 to-red-900/90
              backdrop-blur-xl border-r border-red-400/20
            ...
            >
                ...
                <div className='flex items-center gap-3 p-3 mt-3 rounded-xl bg-white/10 hover:bg-white/15 transition-all shadow-inner'>
                    <div className='relative w-12 h-12 rounded-full overflow-hidden border-2 border-red-400/60 shadow-lg'> {user.image ? <Image src={user.image} alt='user' fill className='object-cover rounded-full' /> : <User />}</div>
                    <div >
                        <h2 className='text-lg font-semibold text-white'>{user.name}</h2>
                        <p className='text-xs text-red-200 capitalize tracking-wide'>{user.role}</p>
                    </div>
                </div>
 ...
 <div className='flex items-center gap-3 text-red-300 font-semibold mt-auto hover:bg-red-500/20 p-3 rounded-lg transition-all' onClick={async ()=>await signOut({callbackUrl:"/"})}>
    ...
    return (
        <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-red-500 to-red-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
            ...
            {user.role == "user" && <form className='hidden md:flex items-center bg-white rounded-full px-4 py-2 w-1/2 max-w-lg shadow-md' onSubmit={handleSearch}>
                ...
                <input type="text" placeholder='Search ...' className='w-full outline-none text-gray-700 placeholder-gray-400' 
                value={search}
            ...
            <div className='flex items-center gap-3 md:gap-6 relative'>
                ...
                {user.role == "admin" && <>
                    ...
                    <div className='md:hidden bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-md' onClick={() => setMenuOpen(prev => !prev)}>
                        <Menu className='text-red-600 w-6 h-6' />
                    </div>
                </>}
                        ...
                        {open &&
                            ...
                            >
                                <div className='flex items-center gap-3 px-3 py-2 border-b border-gray-100'>
                                    <div className='w-10 h-10 relative rounded-full bg-red-100 flex items-center justify-center overflow-hidden'>
                                        {user.image ? <Image src={user.image} alt='user' fill className='object-cover rounded-full' /> : <User />}
                                    </div>
                                    <div>
                                ...
                                {user.role == "user" && <Link href={"/user/my-orders"} className='flex items-center gap-2 px-3 py-3 hover:bg-red-50 rounded-lg text-gray-700 font-medium' onClick={() => setOpen(false)}>
                        ...
                        {searchBarOpen
                            ...
                            >
                                ...
                                <form className='grow' onSubmit={handleSearch}>
                                    <input type="text" className='w-full outline-none text-gray-700' placeholder='search ...'  value={search}
                onChange={(e)=>setSearch(e.target.value)}/>
                                </form>
...
export default Nav
// 'use client'
// import { Search, ShoppingCartIcon, User, X } from 'lucide-react'
// import Link from 'next/link'
...
//     return (
//         <div className='w-[95%] sticky top-4 mx-auto bg-gradient-to-r from-red-500 to-red-700 rounded-2xl shadow-lg flex justify-between items-center h-20 px-4 md:px-8 z-50'>
...
//                 {/* Mobile Search */}
//                 <div className='md:hidden bg-white w-11 h-11 flex items-center justify-center rounded-full'
//                     onClick={() => setSearchBarOpen(prev => !prev)}>
//                     <Search className='text-red-600' />
//                 </div>

//                 {/* Cart */}
//                 <Link href={"/user/cart"} className='relative bg-white w-11 h-11 flex items-center justify-center rounded-full'>
...
//                 {/* Profile */}
//                 <div className='bg-white w-11 h-11 rounded-full flex items-center justify-center overflow-hidden'>
//                     {user?.image ? <Image src={user.image} alt='user' fill /> : <User />}
//                 </div>
...
//                             exit={{ opacity: 0, y: -10 }}
//                             className='absolute top-full left-0 mt-3 w-[250px] bg-white rounded-full shadow-lg flex items-center px-4 py-2 md:hidden'
//                         >
//                             <Search className='w-5 h-5 mr-2 text-gray-500' />
...
//                                 />
//                             </form>
//                             <X onClick={() => setSearchBarOpen(false)} />
//                         </motion.div>
//                     )}
//                 </AnimatePresence>
//             </div>
